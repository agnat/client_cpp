  ...
  namespace impl {
    ...
    protected:
      ...
      set_virtual_memory(collection_type & l, double value) {
        set_gauge(l, "process_virtual_memory_bytes",
            "Virtual memory size in bytes (vsize)", value);
      }

      static
      void
      set_resident_memory(collection_type & l, double value) {
        set_gauge(l, "process_resident_memory_bytes",
            "Resident memory size in bytes (rss)", value);
      }

      static
      void
      set_start_time(collection_type & l, double value) {
        set_gauge(l, "process_start_time_seconds",
            "Start time of the process since unix epoch in seconds.", value);
      }

      static
      void
      set_cpu_time(collection_type & l, double value) {
        set_gauge(l, "process_cpu_seconds_total",
            "Total user and system CPU time spent in seconds.", value);
      }

      static
      void
      set_open_fds(collection_type & l, double value) {
        set_gauge(l, "process_open_fds",
            "Number of open file descriptors.", value);
      }

      static
      void
      set_max_fds(collection_type & l, double value) {
        set_gauge(l, "process_max_fds",
            "Maximum number of open file descriptors.", value);
      }
